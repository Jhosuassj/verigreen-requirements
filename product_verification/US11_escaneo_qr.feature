Feature: Escaneo de código QR
  ...
  Scenario: Intento de escaneo sin permisos de cámara otorgados
    Given que el consumidor no ha otorgado permisos de acceso a la cámara
    When el consumidor intenta acceder a la función de escaneo
    Then el sistema bloquea el acceso a la cámara
    And muestra el mensaje "VeriGreen necesita acceso a la cámara para escanear códigos QR"
